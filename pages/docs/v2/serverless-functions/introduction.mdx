...
export const meta = {
  ...
  lastEdited: '2019-10-18T03:56:39.000Z'
}

ZEIT Now deployments support various backend languages that you can deploy as Serverless functions.

Serverless functions are pieces of code that run on demand by reacting to events, such as an HTTP event.
They open the door to an entire world of scalable backend micro services that augment your static website.

You can use serverless functions to handle user authentication, form handling, dynamic pdf generation, custom
slack commands, and more. You can get a more in-depth overview of this new compute model by visiting the [Deployment Concepts](https://zeit.co/docs/v2/advanced/concepts/serverless-functions/) section.
